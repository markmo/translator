...
/**
  ...
  */
@Api(value = "/translate-wva-liveengage/{sender}", produces = "application/json")
@Path("/translate-wva-liveengage/{sender}")
class TranslateWva2LiveEngageService(liveEngageTranslationActor: ActorRef,
                                     log: LoggingAdapter
  ...
  val routes = translateWva2LiveEngage
  ...
  def translateWva2LiveEngage =
    post {
      pathPrefix("translate-wva-liveengage") {
        path(Segment) { sender =>
          entity(as[WvaMessageResponse]) { msg =>
            log.debug("received body:\n{}", msg.toJson.prettyPrint)
            val f = liveEngageTranslationActor ? WvaPayload(sender, msg)
            complete {
