package com.telstra.daas.ai.harold.translator.models
...
case class WvaPayload(sender: String, message: WvaMessageResponse)
...
trait WvaJsonSupport extends DefaultJsonProtocol with SprayJsonSupport {
  ...
  implicit val wvaPayloadJsonFormat = jsonFormat2(WvaPayload)
}